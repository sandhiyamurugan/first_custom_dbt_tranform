...
select
    jsonb_extract_path_text(_airbyte_data, 'EMP_ID') as "ID",
...
select
    md5(cast(coalesce(cast("EMP_ID" as text), '') || '-' || coalesce(cast(age as text), '') || '-' || coalesce(cast("NAME" as text), '') || '-' || coalesce(cast(salary as text), '') || '-' || coalesce(cast(updated_at as text), '') as text)) as _airbyte_company_hashid,
    tmp.*
